...
function applyDeadzone(value: number, inner: number, outer: number): number {
    ...
    // the outer deadzone is passed as the distance from the edge
    outer = 1 - outer;
...
function circleRectCollision(circle: Ball, rect: BoundingBox, transVec?: p5.Vector) {
    ...
    const distSq = (
        Math.pow(circle.position.x - closest.x, 2) + Math.pow(circle.position.y - closest.y, 2)
    );
    ...
    if (transVec) { transVec.set(0, 0); }
    return false;
}

namespace Physics {
    export function init(p5_: p5) {
        p5 = p5_;
        ball = {
    ...
    export function update(dt: number, xAngle: number, yAngle: number) {
        ...
        const gravity = p5.createVector(
            applyDeadzone(yAngle, CONFIG.GYRO_INNER_DEADZONE, CONFIG.GYRO_OUTER_DEADZONE),
            applyDeadzone(xAngle, CONFIG.GYRO_INNER_DEADZONE, CONFIG.GYRO_OUTER_DEADZONE)
        ).limit(1).mult(CONFIG.MAX_GRAVITY * dt * 0.1);
        ...
        for (const wall of walls) {
            if (circleRectCollision(ball, wall.bbox, transVec)) {
                ...
                ball.velocity.set(
                    (ball.velocity.x - k * transVec.x) * CONFIG.BALL_ELASTICITY,
                    (ball.velocity.y - k * transVec.y) * CONFIG.BALL_ELASTICITY
                );
            }
        }
